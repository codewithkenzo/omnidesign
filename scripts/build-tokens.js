...
function generateCSS(mergedTokens) {
	...
	log(
		`CSS: Generated ${count} variables, skipped ${skipped} composite tokens`,
		"green",
	);
...
function generateTypeScript(mergedTokens, themes) {
	...
	for (let i = 0; i < categories.length; i++) {
		...
		const isLast =
			i === categories.length - 1 && Object.keys(themes).length === 0;

		lines.push(
			`  ${category}: ${generateTSObject(tokens, 2)}${isLast ? "" : ","}`,
		);
	}
	...
	if (Object.keys(themes).length > 0) {
		...
		for (let i = 0; i < themeNames.length; i++) {
			...
			lines.push(
				`    "${themeName}": ${generateTSObject(themeTokens, 3)}${isLast ? "" : ","}`,
			);
		}
		lines.push("  }");
	}
	...
	for (let i = 0; i < entries.length; i++) {
		...
		if (value && typeof value === "object") {
			if ("$value" in value) {
				const tokenValue = value.$value;
				lines.push(
					`${innerSpaces}${safeKey}: ${JSON.stringify(tokenValue)}${comma}`,
				);
			} else {
				lines.push(
					`${innerSpaces}${safeKey}: ${generateTSObject(value, indent + 1)}${comma}`,
				);
			}
		} else {
