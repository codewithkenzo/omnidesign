...
program
	...
	.option(
		"-i, --ide <ide>",
		"Target IDE (claude, cursor, opencode, vscode, aider, continue, zed, amp, kilo, antigravity)",
	)
	.option(
		"-g, --global",
		"Install to global IDE config (~/.claude, ~/.cursor, etc.)",
	)
	.action(async (options) => {
		...
		if (!detectedIde) {
			console.log(
				chalk.yellow(
					"⚠️  Could not auto-detect IDE. Please specify with --ide flag",
				),
			);
			console.log(
				chalk.gray(
					"Supported: claude, cursor, opencode, vscode, aider, continue, zed, amp, kilo, antigravity",
				),
			);
			process.exit(1);
		}
	...
	.action((options) => {
		...
		const ides = [
			...
			{ name: "Cursor", cmd: "cursor", ...getCursorPaths(options.global) },
			{
				name: "OpenCode",
				cmd: "opencode",
				...getOpenCodePaths(options.global),
			},
			...
			{ name: "Kilo Code", cmd: "kilo", ...getKiloPaths(options.global) },
			{
				name: "Antigravity",
				cmd: "antigravity",
				...getAntigravityPaths(options.global),
			},
			{ name: "Aider", cmd: "aider", ...getAiderPaths(options.global) },
			{
				name: "Continue.dev",
				cmd: "continue",
				...getContinuePaths(options.global),
			},
		];
		...
		if (fs.existsSync(path.join(homedir, ".config", "opencode")))
			detected.push("opencode");
		if (fs.existsSync(path.join(homedir, ".config", "agents")))
			detected.push("amp");
		if (fs.existsSync(path.join(homedir, ".config", "Code")))
			detected.push("vscode");
		if (fs.existsSync(path.join(homedir, ".config", "zed")))
			detected.push("zed");
		if (fs.existsSync(path.join(homedir, ".kilocode"))) detected.push("kilo");
		if (fs.existsSync(path.join(homedir, ".gemini", "antigravity")))
			detected.push("antigravity");
		if (fs.existsSync(path.join(homedir, ".continue")))
...
function getClaudePaths(global = false) {
	...
	return {
		...
		configFile: path.relative(
			process.cwd(),
			path.join(baseDir, "skills", "omnidesign", "SKILL.md"),
		),
		baseDir,
	};
}
...
function getZedPaths(global = false) {
	const baseDir = global
		...
		: path.join(process.cwd(), ".zed");
	return {
		configPath: path.join(baseDir, "omnidesign.json"),
		configFile: path.relative(
			process.cwd(),
			path.join(baseDir, "omnidesign.json"),
		),
		baseDir,
	};
}
...
function getContinuePaths(global = false) {
	...
	return {
		...
		configFile: path.relative(
			process.cwd(),
			path.join(baseDir, "omnidesign.yaml"),
		),
		baseDir,
	};
}
...
function detectIDE(global = false) {
	...
	const checks = [
		...
		{
			name: "opencode",
			path: path.join(homedir, ".config", "opencode"),
			global: true,
		},
		{
			name: "vscode",
			path: path.join(homedir, ".config", "Code"),
			global: true,
		},
		{ name: "zed", path: path.join(homedir, ".config", "zed"), global: true },
		{
			name: "amp",
			path: path.join(homedir, ".config", "agents"),
			global: true,
		},
		{ name: "kilo", path: path.join(homedir, ".kilocode"), global: true },
		{
			name: "antigravity",
			path: path.join(homedir, ".gemini", "antigravity"),
			global: true,
		},
		...
		{
			name: "antigravity",
			path: path.join(cwd, ".antigravity"),
			global: false,
		},
	...
	for (const check of checks) {
		if (
			...
			!detected.find((d) => d.name === check.name)
		) {
	...
	console.log(
		chalk.gray(`To use different: npx omnidesign install --ide <name>\n`),
	);
...
function copySkillContent(skillDir, targetDir, packageDir) {
	...
	if (fs.existsSync(tokensSrc)) {
		...
		console.log(
			chalk.gray(
				`  Copied: tokens/ → ${path.relative(process.cwd(), tokensDest)}`,
			),
		);
	}
	...
	if (fs.existsSync(recipesSrc)) {
		...
		console.log(
			chalk.gray(
				`  Copied: recipes/ → ${path.relative(process.cwd(), recipesDest)}`,
			),
		);
	}
}
...
async function installClaudeSkill(skillsDir, packageDir, global) {
	...
	if (fs.existsSync(skillMdSource)) {
		...
		console.log(
			chalk.gray(`  Created: ${path.relative(process.cwd(), skillMdTarget)}`),
		);
	}
	...
	if (!marketplace.plugins.find((p) => p.name === "omnidesign")) {
		marketplace.plugins.push({
			...
			description:
				"Universal design system with 25 themes, tokens, recipes, and AI components",
			version: packageJson.version,
		});
		fs.writeFileSync(marketplacePath, JSON.stringify(marketplace, null, 2));
		console.log(
			chalk.gray(`  Updated: ${path.relative(process.cwd(), marketplacePath)}`),
		);
	}
}
...
async function installOpenCodeSkill(skillsDir, packageDir, global) {
	...
	if (!config.plugins.includes("omnidesign")) {
		...
		console.log(
			chalk.gray(
				`  Updated: ${path.relative(process.cwd(), paths.configJsonPath)}`,
			),
		);
	}
}
...
async function installVSCodeSkill(skillsDir, packageDir, global) {
	...
	console.log(
		chalk.yellow("  Note: VS Code extension coming soon. Settings configured."),
	);
}
...
async function installAiderSkill(skillsDir, packageDir, global) {
	...
	console.log(
		chalk.gray(
			`  Created: ${path.relative(process.cwd(), targetDir)}/ (tokens, recipes)`,
		),
	);
	...
	if (fs.existsSync(configPath)) {
		...
		if (!config.includes("CONVENTIONS.md")) {
			...
			console.log(
				chalk.gray(`  Updated: ${path.relative(process.cwd(), configPath)}`),
			);
		}
	}
}
...
async function uninstallSkill(ide, global = false) {
	switch (ide) {
		case "claude": {
			const paths = getClaudePaths(global);
			fs.rmSync(path.dirname(paths.configPath), {
				force: true,
				recursive: true,
			});
			break;
		...
		case "zed": {
			const paths = getZedPaths(global);
			fs.rmSync(path.join(paths.baseDir, "omnidesign"), {
				force: true,
				recursive: true,
			});
