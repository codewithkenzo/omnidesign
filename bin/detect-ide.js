...
function detectAllIDEs() {
	...
	for (const check of checks) {
		if (
			...
			detected.push({
				name: check.name,
				source: fs.statSync(check.path).isDirectory() ? "global" : "project",
			});
		}
	}
...
function autoInstall() {
	...
	if (detected.length === 0) {
		console.log(
			chalk.gray("ℹ️  No IDE detected. Run: npx omnidesign install --ide <ide>"),
		);
		console.log(
			chalk.gray(
				"Supported: claude, cursor, opencode, vscode, zed, amp, kilo, antigravity, aider, continue",
			),
		);
		return;
	}
	...
	if (detected.length === 1) {
		console.log(
			chalk.blue(`🎨 OmniDesign detected ${detected[0].name}. Installing...\n`),
		);
		try {
			execSync(`node ${path.join(__dirname, "cli.js")} install --global`, {
				stdio: "inherit",
			});
		} catch (error) {
			console.log(
				chalk.gray("ℹ️  Run manually: npx omnidesign install --global"),
			);
		}
		return;
	}
	...
	console.log(
		chalk.gray(`Or specify one:  npx omnidesign install --ide <name> --global`),
	);
}
